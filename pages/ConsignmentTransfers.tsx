...
import { PREFIX_CONSIGNMENT_TRANSFER } from '../constants';
...
const ConsignmentTransfers: React.FC = () => {
    const { user } = useAuth();
    ...
    useEffect(() => {
        setControlNumber(generateControlNumber(PREFIX_CONSIGNMENT_TRANSFER, transferLogs.length));
    }, [transferLogs.length]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
        ...
        await logAuditEvent(user, 'Consignment Stock Transfer Initiate', {
            controlNumber,
        ...
        for(const notifiedUser of usersToNotify) {
            await addNotification({
                ...
                message: `Incoming consignment transfer ${controlNumber} from ${getFacilityName(fromFacilityId)}.`,
                link: '/consignment/transfers',
                type: 'stockTransfer',
  ...
  const handleSaveAcknowledgement = async (logId: string, receivedItems: { inventoryItemId: string, quantity: number }[], notes: string, isDiscrepancy: boolean) => {
    ...
    try {
        ...
        await logAuditEvent(user, 'Consignment Stock Transfer Acknowledge', {
        ...
        await addNotification({
            ...
            message: `Your consignment transfer ${controlNumber} has been acknowledged.`,
            link: '/consignment/transfers',
            type: 'stockTransfer'
        });
  ...
  const getStatusPill = (status: TransferStatus) => {
    ...
    const handlePrintHistory = () => {
        navigate('/print/consignment-transfer-history', { state: { items: sortedHistoryItems } });
    };
    ...
    const inventoryOptionsForSelect = useMemo(() => {
        ...
        const sortedAndMarkedItems = (Object.values(groupedByMaster) as (typeof accessibleItems)[]).flatMap(group => {
  ...
  return (
    <div className="space-y-6">
      <h2 className="text-3xl font-semibold text-secondary-800">Consignment Stock Transfers</h2>
                  ...
                  {paginatedHistoryItems.map(log => (
                       <tr key={log.id}>
                         ...
                         <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium space-x-1">
                            <Button variant="ghost" size="sm" onClick={() => window.open(`/#/print/consignment-transfer/${log.id}`, '_blank')} aria-label="Print" title="Print Voucher"><PrintIcon /></Button>
...
export default ConsignmentTransfers;

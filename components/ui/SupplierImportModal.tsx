...
import { sampleSuppliersCsv } from '../../data/sampleCsvData';

interface SupplierImportModalProps {
  isOpen: boolean;
  onClose: () => void;
  onImport: (items: Omit<Supplier, 'id'>[]) => void;
}
...
const SupplierImportModal: React.FC<SupplierImportModalProps> = ({ isOpen, onClose, onImport }) => {
    ...
    const parseAndValidateCsv = (csvFile: File) => {
        ...
        reader.onload = (e) => {
            ...
            for (let i = 1; i < lines.length; i++) {
                ...
                const finalItemData: Omit<Supplier, 'id'> = { 
                    name: rowData.name || '',
                ...
                {!file && (
                    <>
                        <div className="p-4 bg-secondary-50 border border-secondary-200 rounded-md">
                            ...
                            <p className="text-sm text-secondary-600 mt-1">All imported suppliers will be set to 'Active' status by default.</p>
                             <a
                                href="#"
                                onClick={(e) => {
                                  e.preventDefault();
                                  downloadStringAsFile(sampleSuppliersCsv, 'sample-suppliers.csv', 'text/csv;charset=utf-8;');
                                }}

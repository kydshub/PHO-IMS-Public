...
const DownloadIcon = () => <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="17 8 12 3 7 8"/><line x1="12" x2="12" y1="3" y2="15"/></svg>;

type WriteOffItemRow = { id: string; inventoryItemId: string; quantity: string };

const ConsignmentWriteOffs: React.FC = () => {
    const { user } = useAuth();
    ...
    const inventoryOptions = useMemo(() => {
        ...
        const sortedAndMarkedItems = Object.values(groupedByMaster).flatMap(group => {
            // FIX: Cast 'group' to 'any[]' to resolve 'sort' does not exist on type 'unknown' error.
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (!reason || items.some(i => !i.inventoryItemId || !i.quantity || parseInt(i.quantity) <= 0)) {
            alert('Please fill out the reason and ensure all items have a valid quantity.');
            return;
        }
        if (!user) {
        ...
        for (const [index, itemRow] of items.entries()) {
            ...
            if (!item || parseInt(itemRow.quantity) > item.quantity) {
                alert(`Write-off quantity for ${master?.name || 'an item'} cannot exceed available stock.`);
                return;
            }

            const currentItemFacilityId = storageLocationMap.get(item.storageLocationId);
            if (!currentItemFacilityId) {
                alert(`Could not determine facility for item: ${master?.name}.`);
                return;
            }
            ...
            if (index === 0) {
                ...
                alert('All items in a single write-off transaction must be from the same facility.');
                return;
            }
        }
        ...
        if (reason === WriteOffReason.Wastage && !subReason) {
             alert('Please select a sub-reason for wastage.');
             return;
        }
        if (reason === WriteOffReason.Stolen && !notes) {
             alert('Please provide an incident report ID or notes for stolen items.');
             return;
        }
        ...
        const isConfirmed = await confirm({
            title: "Confirm Consignment Write-Off",
            message: (
                <div>
                    <p className="mb-4">Please review the details below. This will remove the items from inventory and log them as consumed.</p>
        ...
        try {
            ...
            const consumedItems: {inventoryItem: InventoryItem, master: ItemMaster, quantity: number}[] = [];
            ...
            newLogData.items.forEach(logItem => {
                ...
                if(sourceItem && master) {
                    ...
                    if (sourceItem.isConsignment) {
                        consumedItems.push({ inventoryItem: sourceItem, master, quantity: logItem.quantity });
                    }
                }
            });
            
            if (consumedItems.length > 0) {
                const itemsBySupplier = new Map<string, typeof consumedItems>();
                consumedItems.forEach(itemData => {
            ...
            await logAuditEvent(user, 'Consignment Stock Write-Off', {
                controlNumber,
    ...
    const augmentedLogs: (WriteOffLog & { userName: string; facilityName: string; })[] = useMemo(() => {
    ...
    const handlePrintHistory = () => {
        navigate('/print/write-off-history', { state: { items: sortedLogs, title: "Consignment Write-Off History" } });
    };
...
export default ConsignmentWriteOffs;
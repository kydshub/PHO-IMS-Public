...
import { PREFIX_CONSIGNMENT_RETURN } from '../constants';
...
const ConsignmentReturns: React.FC = () => {
    const { user } = useAuth();
    ...
    useEffect(() => {
        setControlNumber(generateControlNumber(PREFIX_CONSIGNMENT_RETURN, returnLogs.length));
    }, [returnLogs.length]);
    ...
    const handleItemChange = (id: string, field: keyof ReturnItemRow, value: string | null) => {
        setItems(prevItems => prevItems.map(item => {
            if (item.id === id) {
                ...
                // FIX: Cast updatedItem to 'any' to resolve 'Type 'unknown' cannot be used as an index type' error.
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            await logAuditEvent(user, 'Consignment Stock Return', {
                controlNumber,
                reason,
    ...
    const filteredLogs: (ReturnLog & { userName: string; facilityName: string; supplierName: string; })[] = useMemo(() => {
        ...
        return augmentedLogs.filter(log => {
            ...
            const supplierMatch = !supplierId || log.supplierId === supplierId;
            const reasonMatch = !reason || log.reason === reason;
            const dateMatch = (!sDate || logDate >= sDate) && (!eDate || logDate <= eDate);
            
            return searchMatch && facilityMatch && supplierMatch && reasonMatch && dateMatch;
        });
    ...
    const exportHistoryToCSV = () => {
        ...
        downloadStringAsFile(csvContent, 'consignment_return_history.csv', 'text/csv;charset=utf-8;');
    };
    
    const handlePrintHistory = () => {
        navigate('/print/consignment-return-history', { state: { items: sortedLogs } });
    };
    ...
    return (
        <div className="space-y-6">
            <h2 className="text-3xl font-semibold text-secondary-800">Return Consignment Items to Supplier</h2>
            ...
            <Card 
                title="Consignment Return History"
                footer={
                 <TablePagination
                            ...
                            {paginatedLogs.map((log, index) => (
                                <tr key={log.id} className={`${index % 2 === 0 ? 'bg-white' : 'bg-secondary-50/50'} hover:bg-primary-50`}>
                                    ...
                                    <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium space-x-1">
                                        <Button variant="ghost" size="sm" onClick={() => window.open(`/#/print/consignment-return/${log.id}`, '_blank')} aria-label="Print" title="Print Voucher"><PrintIcon /></Button>
...
export default ConsignmentReturns;
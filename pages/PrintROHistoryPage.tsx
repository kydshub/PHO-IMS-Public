...
const PrintROHistoryPage: React.FC = () => {
    const location = useLocation();
    const { settings } = useSettings();
    const { items: roLogs } = location.state || {};
        ...
        return (
            <PrintLayout title="Error: Report Data Not Found">
                <div className="flex flex-col items-center justify-center text-center">
                    ...
                    <p className="mt-2 text-secondary-600">This page cannot be accessed directly. Please generate a report from the Release Order page first.</p>
                    <Link to="/release-order">
                        <Button className="mt-6">Go to Release Order Page</Button>
                    </Link>
                </div>
            </PrintLayout>
        );
    ...
    const ReportHeader = () => (
        <div className="text-center mb-8 pb-4 border-b-2 border-gray-800">
            ...
            <h2 className="text-2xl font-semibold text-gray-700 mt-1">Release Order History Report</h2>
    ...
    return (
        <PrintLayout title={`RO History Report - ${new Date().toLocaleDateString()}`}>
            <ReportHeader />
            <section className="mb-8 break-inside-avoid">
                    <StandardTable headers={["Date", "Control #", "Items Summary", "Ordered To", "Purpose", "Issued By", "Facility"]}>
                       {roLogs.map((log: ROLog) => {
                           ...
                           return (
                                <tr key={log.id} className="border-t">
                                    ...
                                    <td className="p-2 border">{log.orderedTo}</td>
                    ...
                    {roLogs.length === 0 && <p className="text-gray-600 text-sm p-2">No RO records to display.</p>}
            </section>
        </PrintLayout>
    );
};

export default PrintROHistoryPage;
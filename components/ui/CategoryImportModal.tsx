...
import { sampleCategoriesCsv } from '../../data/sampleCsvData';

interface CategoryImportModalProps {
  isOpen: boolean;
  onClose: () => void;
  onImport: (items: Omit<Category, 'id'>[]) => void;
}
...
const CategoryImportModal: React.FC<CategoryImportModalProps> = ({ isOpen, onClose, onImport }) => {
    ...
    const parseAndValidateCsv = (csvFile: File) => {
        ...
        reader.onload = (e) => {
            ...
            for (let i = 1; i < lines.length; i++) {
                ...
                const finalItemData: Omit<Category, 'id'> = { name: rowData.name || '' };
                    ...
                    <>
                        <div className="p-4 bg-secondary-50 border border-secondary-200 rounded-md">
                            <h4 className="font-semibold text-secondary-800">CSV Format Instructions</h4>
                             <p className="text-sm text-secondary-600 mt-1">Your CSV file must contain a header row with the following column: <strong>name</strong>.</p>
                             <a
                                href="#"
                                onClick={(e) => {
                                  e.preventDefault();
                                  downloadStringAsFile(sampleCategoriesCsv, 'sample-categories.csv', 'text/csv;charset=utf-8;');
                                }}
                             ...
                             <table className="min-w-full divide-y divide-secondary-200">
                                ...
                                <tbody className="bg-white divide-y divide-secondary-200">
                                    {parsedData.map((item, index) => (
                                        <tr key={item.rowIndex} className={`${item.errors.length > 0 ? (index % 2 === 0 ? 'bg-red-50' : 'bg-red-100') : (index % 2 === 0 ? 'bg-white' : 'bg-secondary-50/50')} hover:bg-primary-50`}>

...
import { Role, ROLog, InventoryItem, ItemMaster, FacilityStatus, PhysicalCountStatus, ConsignmentConsumptionLog, Facility, Category } from '../types';
...
import { PREFIX_RO } from '../constants';
...
type ROItemRow = { id: string; inventoryItemId: string; quantity: string };

const RO: React.FC = () => {
    const { user } = useAuth();
    const { data } = useDatabase();
    const { roLogs, inventoryItems, itemMasters, storageLocations, users, facilities, physicalCounts, categories } = data;
    const confirm = useConfirmation();
    const { showSuccess, showError } = useInfoModal();
    const navigate = useNavigate();
  
    const [controlNumber, setControlNumber] = useState('');
    const initialItemState: ROItemRow = { id: crypto.randomUUID(), inventoryItemId: '', quantity: '' };
    const [items, setItems] = useState<ROItemRow[]>([initialItemState]);
    ...
    useEffect(() => {
      setControlNumber(generateControlNumber(PREFIX_RO, roLogs.length));
    }, [roLogs.length]);
    ...
    const handleItemChange = (id: string, field: keyof ROItemRow, value: string | null) => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        e.preventDefault();
        if (!orderedTo || !purpose || !recommendingApproval || !approvedBy || items.some(i => !i.inventoryItemId || !i.quantity || parseInt(i.quantity) <= 0)) {
        ...
        for (const [index, itemRow] of items.entries()) {
            const itemDetails = getDetailsForItem(itemRow.inventoryItemId);
            if (!itemDetails.item || parseInt(itemRow.quantity) > itemDetails.item.quantity) {
                showError({title: "Validation Error", message: `Quantity for ${itemDetails.master?.name || 'an item'} cannot exceed available stock.`});
                return;
            }
            
            const currentItemFacilityId = storageLocationMap.get(itemDetails.item.storageLocationId);
            if (!currentItemFacilityId) {
                showError({title: "Validation Error", message: `Could not determine facility for item: ${itemDetails.master?.name}.`});
                return;
            }
        ...
        const isConfirmed = await confirm({
            ...
            message: "This will deduct the items from inventory. Please confirm.",
            confirmText: "Confirm"
        });
        ...
        const newLogRef = db.ref('roLogs').push();
        const logId = newLogRef.key!;
        const newLogData: Omit<ROLog, 'id'> = {
            controlNumber,
            items: items.map(i => ({ inventoryItemId: i.inventoryItemId, quantity: parseInt(i.quantity) })),
            orderedTo, purpose, recommendingApproval, approvedBy, notes,
            userId: user.uid,
        ...
        try {
            const updates: Record<string, any> = { [`/roLogs/${logId}`]: newLogData };
              ...
              const itemsBySupplier = new Map<string, typeof consumedConsignmentItems>();
                ...
                itemsBySupplier.forEach((supplierItems, supplierId) => {
                    ...
                    const newConsumptionLog: Omit<ConsignmentConsumptionLog, 'id'> = {
                        dispenseLogId: `ro-${logId}`,
                        controlNumber: `CONSUME-RO-${controlNumber}`,
                        supplierId,
            ...
            await logAuditEvent(user, 'Release Order Create', { controlNumber, orderedTo, facilityName: getFacilityName(transactionFacilityId) });
            showSuccess({title: "Success", message: "Release Order created successfully!"});
            // reset form state
    ...
    const augmentedLogs: (ROLog & { userName: string; facilityName: string; })[] = useMemo(() => roLogs.map(log => ({ ...log, userName: getUserName(log.userId), facilityName: getFacilityName(log.facilityId) })), [roLogs, users, facilities]);
    
    const filteredLogs: (ROLog & { userName: string; facilityName: string; })[] = useMemo(() => {
        ...
        return augmentedLogs.filter(log => {
            ...
            const searchMatch = !searchTerm || log.controlNumber.toLowerCase().includes(searchTerm.toLowerCase()) || log.orderedTo.toLowerCase().includes(searchTerm.toLowerCase());
    ...
    const exportHistoryToCSV = () => {
        const headers = ['Date', 'Control #', 'Items Summary', 'Ordered To', 'Purpose', 'Issued By', 'Facility'];
        const csvRows = [
            headers.join(','),
            ...sortedLogs.map(log => {
        ...
        downloadStringAsFile(csvContent, 'ro_history.csv', 'text/csv;charset=utf-8;');
    };
    const handlePrintHistory = () => navigate('/print/ro-history', { state: { items: sortedLogs } });
    ...
    return (
        <div className="space-y-6">
            <h2 className="text-3xl font-semibold text-secondary-800">Release Order (RO)</h2>
                    ...
                    <div className="p-6 space-y-6">
                        <div>
                            ...
                            <div className="mt-4 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                                ...
                                <Input label="Ordered To" value={orderedTo} onChange={e => setOrderedTo(e.target.value)} required />
                                <Input label="Purpose" value={purpose} onChange={e => setPurpose(e.target.value)} required />
                            </div>
                            <div className="mt-4 grid grid-cols-1 md:grid-cols-2 gap-4">
                                <Input label="Recommending Approval" value={recommendingApproval} onChange={e => setRecommendingApproval(e.target.value)} required />
                                <Input label="Approved By" value={approvedBy} onChange={e => setApprovedBy(e.target.value)} required />
                            </div>
                             <div className="mt-4">
                              ...
                              {items.map((itemRow, index) => {
                                  ...
                                  return (
                                      <div key={itemRow.id} className="grid grid-cols-12 gap-x-4 gap-y-3 items-end p-2 rounded-lg even:bg-secondary-50/50">
                                          <div className="col-span-12 md:col-span-8"><SearchableSelect options={inventoryOptions} value={itemRow.inventoryItemId} onChange={value => handleItemChange(itemRow.id, 'inventoryItemId', value)} placeholder="Search for an item..." renderOption={renderInventoryOption} /></div>
                     ...
                     <div className="bg-secondary-50 px-6 py-4 text-right rounded-b-lg">
                        <Button type="submit" disabled={isSubmitting}>{isSubmitting ? <Spinner size="sm" /> : 'Record Release Order'}</Button>
                    </div>
                </Card>
            </form>
            
            <Card title="Recent Release Orders" footer={<TablePagination currentPage={currentPage} totalPages={totalPages} itemsPerPage={itemsPerPage} totalItems={sortedLogs.length} startItemIndex={startItemIndex} endItemIndex={endItemIndex} onPageChange={setCurrentPage} onItemsPerPageChange={setItemsPerPage} />}>
                    ...
                    <table className="min-w-full divide-y divide-secondary-200">
                        <thead className="bg-secondary-50">
                            <tr>
                                ...
                                <SortableHeader sortKey="orderedTo" requestSort={requestSort} sortConfig={sortConfig}>Ordered To</SortableHeader>
                            ...
                            {paginatedLogs.map(log => {
                                ...
                                return (
                                    <tr key={log.id}>
                                        ...
                                        <td className="px-6 py-4 whitespace-nowrap text-sm text-secondary-500">{log.orderedTo}</td>
                                        <td className="px-6 py-4 whitespace-nowrap text-sm text-secondary-500">{log.userName}</td>
                                        <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium space-x-1">
                                            <Button variant="ghost" size="sm" onClick={() => window.open(`/#/print/ro/${log.id}`, '_blank')}><PrintIcon /></Button>
                                        </td>
                                    </tr>
                                );
                            })}
                     ...
                     {paginatedLogs.length === 0 && <p className="text-center p-4">No RO have been recorded yet.</p>}
                </div>
            </Card>
        </div>
    );

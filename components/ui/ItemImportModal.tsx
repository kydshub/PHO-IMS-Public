...
import { ItemMaster, Category, ItemType } from '../../types';
import { downloadStringAsFile } from '../../utils/download';
import { sampleItemsCsv } from '../../data/sampleCsvData';

interface ItemImportModalProps {
  isOpen: boolean;
  onClose: () => void;
  onImport: (items: Omit<ItemMaster, 'id'>[]) => void;
  categories: Category[];
}

const REQUIRED_HEADERS = ['name', 'categoryName', 'itemType', 'unit', 'unitCost', 'lowStockThreshold'];

interface ParsedItem {
    data: Omit<ItemMaster, 'id'>;
    rowIndex: number;
    errors: string[];
}

const ItemImportModal: React.FC<ItemImportModalProps> = ({ isOpen, onClose, onImport, categories }) => {
    ...
    const parseAndValidateCsv = (csvFile: File) => {
        ...
        reader.onload = (e) => {
            ...
            for (let i = 1; i < lines.length; i++) {
                ...
                if (!rowData.categoryName) errors.push('"categoryName" is required.');
                if (!rowData.itemType) errors.push('"itemType" is required.');
                if (!rowData.unit) errors.push('"unit" is required.');
                
                const categoryId = categoryMap.get(rowData.categoryName?.toLowerCase());
                if (rowData.categoryName && !categoryId) errors.push(`Category "${rowData.categoryName}" not found.`);

                if (rowData.itemType && !Object.values(ItemType).includes(rowData.itemType as ItemType)) {
                    errors.push(`Invalid itemType "${rowData.itemType}". Must be one of: ${Object.values(ItemType).join(', ')}`);
                }

                const unitCost = parseFloat(rowData.unitCost);
                if (isNaN(unitCost) || unitCost < 0) errors.push('Invalid "unitCost". Must be a non-negative number.');

                const lowStockThresholdStr = rowData.lowStockThreshold?.trim();
                let lowStockThreshold: number | null = null;
                if (lowStockThresholdStr) {
                    const parsed = parseInt(lowStockThresholdStr, 10);
                    if (isNaN(parsed) || parsed < 0) {
                        errors.push('Invalid "lowStockThreshold". Must be a non-negative integer or empty.');
                    } else {
                        lowStockThreshold = parsed;
                    }
                }
                
                const finalItemData: Omit<ItemMaster, 'id'> = {
                    name: rowData.name || '',
                    description: rowData.description || '',
                    categoryId: categoryId || '',
                    unit: rowData.unit || '',
                    lowStockThreshold: lowStockThreshold,
                    itemType: rowData.itemType as ItemType,
                    ...
                    <>
                        <div className="p-4 bg-secondary-50 border border-secondary-200 rounded-md">
                            ...
                            <p className="text-sm text-secondary-600 mt-1">
                                Your CSV file must contain a header row with the following columns. Bold columns are required.
                            </p>
                            <ul className="list-disc list-inside text-sm mt-2 text-secondary-600">
                                {REQUIRED_HEADERS.map(h => <li key={h}><strong>{h}</strong></li>)}
                            </ul>
                             <p className="text-sm text-secondary-600 mt-2">Optional columns: `description`, `brand`, `manufacturer`, `barcode`</p>
                             <p className="text-sm text-secondary-600 mt-2">Note: `categoryName` must match an existing category name exactly (case-insensitive).</p>
                             <a
                                href="#"
                                onClick={(e) => {
                                  e.preventDefault();
                                  downloadStringAsFile(sampleItemsCsv, 'sample-items.csv', 'text/csv;charset=utf-8;');
                                }}

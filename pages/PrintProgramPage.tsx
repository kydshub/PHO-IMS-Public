...
const PrintProgramPage: React.FC = () => {
        ...
        return (
            <PrintLayout title="Error: Report Data Not Found">
                <div className="flex flex-col items-center justify-center text-center">
                    ...
                    <p className="mt-2 text-secondary-600">This page cannot be accessed directly. Please generate a report from the Program Management page first.</p>
                    <Link to="/programs">
                        <Button className="mt-6">Go to Program Management</Button>
                    </Link>
                </div>
            </PrintLayout>
        );
    ...
    const ReportHeader = () => (
        <div className="text-center mb-8 pb-4 border-b-2 border-gray-800">
            ...
            <h2 className="text-2xl font-semibold text-gray-700 mt-1">Program List Report</h2>
    ...
    return (
        <PrintLayout title={`Program Report - ${new Date(generatedDate).toLocaleDateString()}`}>
            <ReportHeader />
            <section className="mb-8 break-inside-avoid">
                <StandardTable headers={["Program Name", "Program Manager", "Email", "Contact Number"]}>
                   {items.map((program: Program) => (
                        <tr key={program.id} className="border-t">
                            <td className="p-2 border font-medium">{program.name}</td>
                            <td className="p-2 border">{program.programManagerName}</td>
                            <td className="p-2 border">{program.programManagerEmail}</td>
                            <td className="p-2 border">{program.programManagerContact}</td>
                        </tr>
                   ))}
                </StandardTable>

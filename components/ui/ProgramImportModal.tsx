...
import { sampleProgramsCsv } from '../../data/sampleCsvData';

interface ProgramImportModalProps {
  isOpen: boolean;
  onClose: () => void;
  onImport: (items: Omit<Program, 'id'>[]) => void;
}

const REQUIRED_HEADERS = ['name', 'programManagerName', 'programManagerEmail', 'programManagerContact'];
...
const ProgramImportModal: React.FC<ProgramImportModalProps> = ({ isOpen, onClose, onImport }) => {
    ...
    const parseAndValidateCsv = (csvFile: File) => {
        ...
        reader.onload = (e) => {
            ...
            for (let i = 1; i < lines.length; i++) {
                ...
                if (!rowData.programManagerName) errors.push('"programManagerName" is required.');
                if (!rowData.programManagerEmail) errors.push('"programManagerEmail" is required.');
                if (!rowData.programManagerContact) errors.push('"programManagerContact" is required.');
                
                const finalItemData: Omit<Program, 'id'> = { 
                    name: rowData.name || '',
                    programManagerName: rowData.programManagerName || '',
                    programManagerEmail: rowData.programManagerEmail || '',
                    programManagerContact: rowData.programManagerContact || '',
                };
                    ...
                    <>
                        <div className="p-4 bg-secondary-50 border border-secondary-200 rounded-md">
                            <h4 className="font-semibold text-secondary-800">CSV Format Instructions</h4>
                             <p className="text-sm text-secondary-600 mt-1">Your CSV file must contain a header row with the following columns: <strong>name, programManagerName, programManagerEmail, programManagerContact</strong>.</p>
                             <a
                                href="#"
                                onClick={(e) => {
                                  e.preventDefault();
                                  downloadStringAsFile(sampleProgramsCsv, 'sample-programs.csv', 'text/csv;charset=utf-8;');
                                }}
